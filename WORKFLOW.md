...
All risky actions require explicit human approval before execution. This includes outbound messaging, web browsing/fetching, UI automation, automation triggers, host execution, and production changes. See `OPENCLAW_GUARDRAILS.md` for the authoritative guardrails.

**Approval request must include**:
- Action summary
- Exact payload or command to be executed
- Target system or scope
- Expected impact and rollback plan
- Where logs will be recorded

**Approval format**:
- Approve: `APPROVE: <short reason>`
- Deny: `DENY: <short reason>`

If approval is not granted, the action is not executed. All approvals must be logged to `agent.persist` with timestamp and action summary.
...
- Goal: short statement of desired outcome
- Scope: what is in and out
- Inputs: files, data sources, assumptions
- Outputs: expected artifacts (file, report, diff)
...
- If blocked, stop and escalate within 24 hours.
...
**Status updates**:
- Every 4 hours for active tasks
- Include current step, blockers, and ETA
